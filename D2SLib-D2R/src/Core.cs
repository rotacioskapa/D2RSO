...
public class Core
{
    ...
    /// <summary>
    /// Selects the data tables (RotW vs pre-RotW) that match the given save/item version and makes
    /// them the active <see cref="MetaData"/>. Call before reading items of a known version.
    /// </summary>
    public static void UseMetaDataForVersion(uint version) =>
        MetaData = ResourceFilesData.Instance.ForVersion(version);
