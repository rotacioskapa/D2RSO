...
public sealed class Item : IDisposable
{
    ...
    public ushort CompactExtra { get; set; } // RotW >=0x69: 9-bit stack qty on AdvancedStashStackable simple items
    ...
    public static Item Read(IBitReader reader, uint version)
    {
        ...
        // RotW (>=0x69): AdvancedStashStackable items (potions, runes, gems, Worldstone Shards…)
        // carry a 9-bit stack-quantity field at the END of the item body — after the compact data
        // for simple items, after the stat lists for extended items.
        if (version >= 0x69 && IsAdvancedStashStackable(item.Code))
        {
            item.CompactExtra = reader.ReadUInt16(9);
        }
        // RotW (>=0x69): extended items advance to a STRICTLY-next byte boundary
        // (a full padding byte is present even when already byte-aligned).
        if (version >= 0x69 && !item.IsCompact && reader.Position % 8 == 0)
        {
            reader.AdvanceBits(8);
        }
        reader.Align();
    ...
    private static bool IsAdvancedStashStackable(string code)
    {
        var misc = Core.MetaData.ItemsData.MiscData;
        if (!misc.ColumnNames.ContainsKey("AdvancedStashStackable")) { return false; }
        var row = misc[code];
        return row is not null && row["AdvancedStashStackable"].ToBool();
    }
    ...
    private static void WriteCompact(IBitWriter writer, Item item, uint version)
    {
        ...
        else
        {
            ...
            if (version >= 0x69 && IsAdvancedStashStackable(item.Code))
            {
                writer.WriteUInt16(item.CompactExtra, 9);
            }
        }
    }
    ...
    private static void ReadComplete(IBitReader reader, Item item, uint version)
    {
        ...
        if (isArmor || isWeapon)
        {
            ...
            {
                // Current-durability width differs by version (the two forms both span 17 bits total):
                //   pre-RotW: 8-bit value (maxdurability width) + a 1-bit trailing field.
                //   RotW (>=0x69): the full 9-bit durability stat, with no trailing bit.
                var curDurabilityStat = version >= 0x69 ? durabilityStat : maxDurabilityStat;
                item.Durability = (ushort)(reader.ReadUInt16(curDurabilityStat?["Save Bits"].ToInt32() ?? 0) + curDurabilityStat?["Save Add"].ToUInt16() ?? 0);
                if (version < 0x69) { reader.ReadBit(); }
            }
        }
        // RotW (>=0x69): a single 1-bit "mystery" field precedes the stat lists for EVERY
        // extended item (generalizes the old durability-only bit). Items with durability used
        // to consume it inside the block above; now every extended item reads exactly one here.
        if (version >= 0x69) { reader.ReadBit(); }
        if (isStackable)
        {
...
public class ItemStatList
{
    ...
    public static ItemStatList Read(IBitReader reader)
    {
        ...
        while (reader.Position + 9 <= reader.Length)
        {
