...
public class ClassSkills
{
    ...
    private static readonly uint[] SKILL_OFFSETS = { 6, 36, 66, 96, 126, 221, 251, 281 }; // 281 = Warlock (tentative; verify against skills.txt if skill IDs matter)
    ...
    public static ClassSkills Read(IBitReader reader, int playerClass)
    {
        ...
        if (playerClass < 0 || playerClass >= SKILL_OFFSETS.Length)
            throw new InvalidDataException($"Unknown playerClass byte value: {playerClass} (0x{playerClass:X2}). Save file may be misaligned or use an unsupported class.");
